...
Zentry now supports **Graph Memory**.
...
To use Zentry with Graph Memory support, install it using pip:

<CodeGroup>
```bash Python
pip install "zentry[graph]"
```
...
This command installs Zentry along with the necessary dependencies for graph functionality.
...
User can also customize the LLM for Graph Memory from the [Supported LLM list](https://docs.zentry.gg/components/llms/overview) with three levels of configuration:
...
## Graph Operations
The Zentry's graph supports the following operations:

### Add Memories

<Note>
If you are using Zentry with Graph Memory, it is recommended to pass `user_id`. Use `userId` in NodeSDK.
</Note>
...
# Example Usage
Here's an example of how to use Zentry's graph operations:
...
If you want to use a managed version of Zentry, please check out [Zentry](https://zentry.gg/placeholder). If you have any questions, please feel free to reach out to us using one of the following methods:
