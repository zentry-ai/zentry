---
title: Async Memory
description: 'Asynchronous memory for Zentry'
icon: "bolt"
iconType: "solid"
---
...
The `AsyncMemory` class is a direct asynchronous interface to Zentry's in-process memory operations. Unlike the memory, which interacts with an API, `AsyncMemory` works directly with the underlying storage systems. This makes it ideal for applications where you want to embed Zentry directly into your codebase.

### Initialization

To use `AsyncMemory`, import it from the `zentry.memory` module:
