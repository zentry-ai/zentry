...
Create voice-based conversational AI agents with memory capabilities by integrating ElevenLabs and Zentry. This integration enables persistent, context-aware voice interactions that remember past conversations.

## Overview

In this guide, we'll build a voice agent that:
1. Uses ElevenLabs Conversational AI for voice interaction
2. Leverages Zentry to store and retrieve memories from past conversations
...
```bash
pip install elevenlabs Zentry python-dotenv
```

Configure your environment variables:

<Note>You'll need both an ElevenLabs API key and a Zentry API key to use this integration.</Note>
...
- `AsyncMemoryClient` from Zentry for memory operations
...
def main():
    ...
    Zentry_API_KEY = os.environ.get('Zentry_API_KEY')
    ...
    if not Zentry_API_KEY:
        sys.stderr.write("Zentry_API_KEY environment variable must be set\n")
        sys.exit(1)

    # Set up Zentry API key in the environment
...
- Sets the Zentry API key in the environment for the Zentry client to use

### 3. Client Initialization

Initialize both the ElevenLabs and Zentry clients:
...
```python
    ...
    async def retrieve_memories(parameters):
        ...
        results = await Zentry_client.search(
            query=message,
            version="v2", 
            filters=filters
        )
...
- Stores the message in Zentry using the `add` method
...
When the agent identifies information worth remembering, it calls this function to store it in the Zentry database with the appropriate user ID.
...
4. The function stores the memory in Zentry linked to the user's ID
...
3. The function performs a semantic search in Zentry
...
By integrating ElevenLabs Conversational AI with Zentry, you can create voice agents that maintain context across conversations and provide personalized responses based on user history. This powerful combination enables:
