...
Zentry is a self-improving memory layer for LLM applications, enabling personalized AI experiences that save costs and delight users. Keywords AI provides complete LLM observability.

Combining Zentry with Keywords AI allows you to:
...
<Note>
You can get your Zentry API key, user_id, and org_id from the [Zentry dashboard](https://app.zentry.gg/). These are required for proper integration.
</Note>
...
```bash
pip install Zentry keywordsai-sdk
```
...
Here's a simple example of using Zentry with Keywords AI:
...
# Set up Zentry with Keywords AI as the LLM provider
config = {
    "llm": {
        "provider": "openai",
        "config": {
...
response = client.chat.completions.create(
    ...
    extra_body={
        "Zentry_params": {
            "user_id": "test_user",
            "org_id": "org_1",
            "api_key": os.environ.get("Zentry_API_KEY"),
            "add_memories": {
                "messages": messages,
            },
        }
...
For detailed information on this integration, refer to the official [Keywords AI Zentry integration documentation](https://docs.keywordsai.co/integration/development-frameworks/Zentry).
...
Integrating Zentry with Keywords AI provides a powerful combination for building AI applications with persistent memory and comprehensive observability. This integration enables more personalized user experiences while providing insights into your application's memory usage.

## Help

For more information on using Zentry and Keywords AI together, refer to:
