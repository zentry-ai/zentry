...
2. Leverages Zentry to store and retrieve conversation history
...
```bash
pip install crewai crewai-tools Zentryai
```

Import required modules and set up configurations:

<Note>Remember to get your API keys from [Zentry Platform](https://app.zentry.gg), [OpenAI](https://platform.openai.com) and [Serper Dev](https://serper.dev) for search capabilities.</Note>
...
def setup_crew(agents: list, tasks: list):
    """Set up a crew with Zentry memory integration"""
    return Crew(
        agents=agents,
        tasks=tasks,
...
1. **Persistent Memory**: Uses Zentry to maintain user preferences and conversation history
