...
Webhooks enable real-time notifications for memory events in your Zentry project. Webhooks are configured at the project level, meaning each webhook is tied to a specific project and receives events solely from that project. You can configure webhooks to send HTTP POST requests to your specified URLs whenever memories are created, updated, or deleted.
...
const { MemoryClient } = require('Zentryai');
...
Zentry supports the following event types for webhooks:
...
When a memory event occurs, Zentry sends an HTTP POST request to your webhook URL with the following payload:
...
2. **Verify Webhook Source**: Implement security measures to verify that webhook requests originate from Zentry.
