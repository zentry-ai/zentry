...
import { MemoryClient, Memory as zentryMemory } from 'zentryai';
...
export const useChat = ({ user, zentryApiKey, openaiApiKey }: UseChatProps): UseChatReturn => {
  ...
  const updateMemories = async (messages: PromptMessage[]) => {
    const memoryClient = new MemoryClient({ apiKey: zentryApiKey || '' });
    try {
      ...
      const newMemories = response.map((memory: zentryMemory) => ({
        id: memory.id || '',
      ...
      let relevantMemories = '';
        try {
          ...
          relevantMemories = searchResponse
            .map((memory: zentryMemory) => `Previous context: ${memory.memory}`)
            .join('\n');
        } catch (error) {
