...
import { createZentry, getMemories } from '@zentry/vercel-ai-provider';
...
export const useChat = ({ user, zentryApiKey, openaiApiKey, provider }: UseChatProps): UseChatReturn => {
  ...
  const sendMessage = async (content: string, fileData?: { type: string; data: string | Buffer }) => {
    ...
    try {
      const { textStream } = await streamText({
        model: zentry(AI_MODELS[provider], {
          user_id: user,
        }),
