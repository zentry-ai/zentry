...
const config = {
  ...
  zentryApiKey: "", // Will be set through extension options
};

// Initialize ZentryAI - will be initialized properly when API key is available
let zentryclient = null;
let zentryInitializing = false;

// Function to initialize ZentryAI with API key from storage
async function initializeZentryAI() {
  if (zentryInitializing) return; // Prevent multiple simultaneous initialization attempts
  zentryInitializing = true;

  try {
    // Get API key from storage
    const items = await chrome.storage.sync.get(["zentryApiKey"]);
    if (items.zentryApiKey) {
      try {
        // Create new client instance with v2.1.11 configuration
        zentryclient = new MemoryClient({
          apiKey: items.zentryApiKey,
        ...
        await zentryclient.updateProject({
...
Remember: The goal is to build a comprehensive understanding of both the user's knowledge and their learning journey through YouTube.`,
        ...
        console.error("Error initializing ZentryAI:", error);
        return false;
      }
    } else {
      console.log("No ZentryAI API key found in storage");
      return false;
    }
  } catch (error) {
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  await initializeZentryAI(); // Initialize ZentryAI
});
...
async function sendMessage() {
  ...
  try {
    // If zentryclient is available, store the message as a memory and search for relevant memories
    if (zentryclient) {
      try {
        ...
        await zentryclient.add(
          ...
          {
            user_id: "youtube-assistant-zentry", // Required parameter
            metadata: {
        ...
        const searchResults = await zentryclient.search(userMessage, {
          user_id: "youtube-assistant-zentry", // Required parameter
          limit: 5,
        });
        ...
        console.error("Error with ZentryAI operations:", memoryError);
...
async function loadMemories() {
  try {
    ...
    if (!zentryclient) {
      const initialized = await initializeZentryAI();
      if (!initialized) {
        memoriesContainer.innerHTML =
          '<div class="error">Please set your Zentry API key in the extension options.</div>';
        return;
      }
    }

    const response = await zentryclient.getAll({
      user_id: "youtube-assistant-zentry",
      page: 1,
      page_size: 50,
    });
