...
async function init() {
  ...
  await loadConfig();
  // Initialize ZentryAI and load memories
  await initializeZentryAI();
  await fetchMemories();
}

// Initialize ZentryAI with API key from storage
async function initializeZentryAI() {
  try {
    const response = await chrome.runtime.sendMessage({ action: "getConfig" });
    const zentryApiKey = response.config.zentryApiKey;

    if (!zentryApiKey) {
      showMemoriesError("Please configure your Zentry API key in the popup");
      return false;
    }
    ...
    showMemoriesError("Failed to initialize ZentryAI");
    return false;
  }
}
...
async function fetchMemories() {
  try {
    ...
    const memories = await zentryclient.getAll({
...
async function saveMemory() {
  ...
  try {
    ...
    await zentryclient.update(currentMemory.id, updatedMemory);
...
async function deleteMemory(memoryId) {
  ...
  try {
    ...
    await zentryclient.delete(memoryId);
...
async function addMemory() {
  ...
  try {
    ...
    const result = await zentryclient.add(
      [
        {
          role: "user",
          content: content,
