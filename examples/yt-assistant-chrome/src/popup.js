...
async function init() {
  try {
    ...
    document
      .getElementById("save-zentry-api-key")
      .addEventListener("click", saveZentryApiKey);
    ...
    document
      .getElementById("toggle-zentry-key")
      .addEventListener("click", () =>
        togglePasswordVisibility("zentry-api-key")
      );
...
// Save zentry API key to storage
async function saveZentryApiKey() {
  const apiKeyInput = document.getElementById("zentry-api-key");
  const apiKey = apiKeyInput.value.trim();

  // Show loading status
  showStatus("Saving Zentry API key...", "warning");

  try {
    // Send to background script for saving
    const response = await chrome.runtime.sendMessage({
      action: "saveConfig",
      config: { zentryApiKey: apiKey },
    });

    if (response.error) {
      showStatus(`Error: ${response.error}`, "error");
    } else {
      showStatus("Zentry API key saved successfully", "success");
      loadConfig(); // Refresh the UI
    }
  } catch (error) {
    showStatus(`Error: ${error.message}`, "error");
...
async function loadConfig() {
  try {
    ...
    // Update zentry API key field
    const zentryApiKeyInput = document.getElementById("zentry-api-key");
    if (config.zentryApiKey) {
      zentryApiKeyInput.value = config.zentryApiKey;
      zentryApiKeyInput.type = "password"; // Ensure it's hidden by default
      document.getElementById("zentry-api-key-section").style.display = "block";
      document.getElementById("zentry-status-text").textContent = "Connected";
      document.getElementById("zentry-status-text").style.color =
        "var(--success-color)";
    } else {
      zentryApiKeyInput.value = "";
      document.getElementById("zentry-api-key-section").style.display = "block";
      document.getElementById("zentry-status-text").textContent =
        "Not configured";
      document.getElementById("zentry-status-text").style.color =
        "var(--warning-color)";
    }
