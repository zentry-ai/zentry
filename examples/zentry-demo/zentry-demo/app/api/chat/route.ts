...
import { addMemories, getMemories } from "@zentry/vercel-ai-provider";
...
export async function POST(req: Request) {
  ...
  const zentryInstructions = retrieveMemories(memories);

  const result = streamText({
    model: openai("gpt-4o"),
    messages,
    // forward system prompt and tools from the frontend
    system: [SYSTEM_HIGHLIGHT_PROMPT, system, zentryInstructions].filter(Boolean).join("\n"),
