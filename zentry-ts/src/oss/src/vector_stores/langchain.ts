...
import { VectorStore } from "./base"; // zentry's VectorStore interface
...
export class LangchainVectorStore implements VectorStore {
  ...
  async insert(
    ...
    const documents = payloads.map((payload, i) => {
      // Provide empty pageContent, store zentry id and other data in metadata
      return new Document({
        pageContent: "", // Add required empty pageContent
        metadata: { ...payload, _zentry_id: ids[i] },
      });
    });

    // Use addVectors. Note: Langchain stores often generate their own internal IDs.
    // We store the zentry ID in the metadata (`_zentry_id`).
    try {
      await this.lcStore.addVectors(vectors, documents, { ids }); // Pass zentry ids if the store supports it
    } catch (e) {
  ...
  async search(
    ...
    // Filters passed via zentry interface are not reliably translatable to generic Langchain stores.
    ...
    // Map Langchain results [Document, score] back to zentry VectorStoreResult
    return results.map(([doc, score]) => ({
      id: doc.metadata._zentry_id || "unknown_id",
  ...
  async get(vectorId: string): Promise<VectorStoreResult | null> {
    ...
    // Perform a search with a filter like { _zentry_id: vectorId }, limit 1.
    // This requires the underlying store to support filtering on _zentry_id.
  }
  ...
  async update(
    ...
    // Possible implementation: Check if store has delete, call delete({_zentry_id: vectorId}), then insert.
  }
  ...
  async delete(vectorId: string): Promise<void> {
    ...
    if (typeof (this.lcStore as any).delete === "function") {
      try {
        // We need to delete based on our stored _zentry_id.
        // Langchain's delete often takes its own internal IDs or filter.
        // Attempting deletion via filter is the most likely approach.
        console.warn(
          "LangchainVectorStore: Attempting delete via filter on '_zentry_id'. Success depends on the specific Langchain VectorStore's delete implementation.",
        );
        await (this.lcStore as any).delete({ filter: { _zentry_id: vectorId } });
