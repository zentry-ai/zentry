...
export default class MemoryClient {
  ...
  _validateApiKey(): any {
    if (!this.apiKey) {
      throw new Error("Zentry API key is required");
    }
    if (typeof this.apiKey !== "string") {
      throw new Error("Zentry API key must be a string");
    }
    if (this.apiKey.trim() === "") {
      throw new Error("Zentry API key cannot be empty");
    }
  }
  ...
  constructor(options: ClientOptions) {
    ...
    this.host = options.host || "https://api.zentry.gg";
  ...
  async _fetchWithErrorHandling(url: string, options: any): Promise<any> {
    const response = await fetch(url, {
      ...
      headers: {
        ...options.headers,
        Authorization: `Token ${this.apiKey}`,
        "Zentry-User-ID": this.telemetryId,
      },
    });
    if (!response.ok) {
