...
from zentry.configs.base import MemoryConfig
...
def memory_instance():
    with (
        patch("zentry.utils.factory.EmbedderFactory") as mock_embedder,
        patch("zentry.utils.factory.VectorStoreFactory") as mock_vector_store,
        patch("zentry.utils.factory.LlmFactory") as mock_llm,
        patch("zentry.memory.telemetry.capture_event"),
        patch("zentry.memory.graph_memory.MemoryGraph"),
    ):
...
def test_custom_prompts(memory_custom_instance):
    ...
    with patch("zentry.memory.main.parse_messages", return_value="Test message") as mock_parse_messages:
        with patch(
            "zentry.memory.main.get_update_memory_messages", return_value="custom update memory prompt"
