...
from zentry.embeddings.vertexai import VertexAIEmbedding


@pytest.fixture
def mock_text_embedding_model():
    with patch("zentry.embeddings.vertexai.TextEmbeddingModel") as mock_model:
        mock_instance = Mock()
...
@pytest.fixture
def mock_os_environ():
    with patch("zentry.embeddings.vertexai.os.environ", {}) as mock_environ:
        yield mock_environ


@pytest.fixture
def mock_config():
    with patch("zentry.configs.embeddings.base.BaseEmbedderConfig") as mock_config:
...
def mock_text_embedding_input():
    with patch("zentry.embeddings.vertexai.TextEmbeddingInput") as mock_input:
        yield mock_input


@patch("zentry.embeddings.vertexai.TextEmbeddingModel")
...
@patch("zentry.embeddings.vertexai.os")
