...
from zentry.configs.vector_stores.azure_ai_search import AzureAISearchConfig

# Import the AzureAISearch class and related models
from zentry.vector_stores.azure_ai_search import AzureAISearch
...
def mock_clients():
    with (
        patch("zentry.vector_stores.azure_ai_search.SearchClient") as MockSearchClient,
        patch("zentry.vector_stores.azure_ai_search.SearchIndexClient") as MockIndexClient,
        patch("zentry.vector_stores.azure_ai_search.AzureKeyCredential") as MockAzureKeyCredential,
    ):
...
def test_config_validation_valid():
    ...
    assert config.collection_name == "zentry"  # Default value
...
def test_initialization(mock_clients):
    ...
    assert "zentry" in mock_search_client._client._config.user_agent_policy.add_user_agent.call_args[0]
    assert "zentry" in mock_index_client._client._config.user_agent_policy.add_user_agent.call_args[0]
