...
from zentry.proxy.main import Chat, Completions, Zentry
...
def mock_openai_embedding_client():
    with patch("zentry.embeddings.openai.OpenAI") as mock_openai:
        mock_client = Mock()
...
def mock_openai_llm_client():
    with patch("zentry.llms.openai.OpenAI") as mock_openai:
        mock_client = Mock()
        mock_openai.return_value = mock_client
        yield mock_client
...
@pytest.fixture
def mock_litellm():
    with patch("zentry.proxy.main.litellm") as mock:
        yield mock


def test_zentry_initialization_with_api_key(mock_openai_embedding_client, mock_openai_llm_client):
    zentry = Zentry()
    assert isinstance(zentry.zentry_client, Memory)
    assert isinstance(zentry.chat, Chat)


def test_zentry_initialization_with_config():
    config = {"some_config": "value"}
    with patch("zentry.Memory.from_config") as mock_from_config:
        zentry = Zentry(config=config)
        mock_from_config.assert_called_once_with(config)
        assert isinstance(zentry.chat, Chat)


def test_zentry_initialization_without_params(mock_openai_embedding_client, mock_openai_llm_client):
    zentry = Zentry()
    assert isinstance(zentry.zentry_client, Memory)
    assert isinstance(zentry.chat, Chat)
