...
class EmbedChain(JSONSerializable):
    ...
    def chat(
        ...
        **kwargs: dict[str, Any],
        ...
        memories = None
        if self.zentry_memory:
            memories = self.zentry_memory.search(
        ...
        # Add to Zentry memory if enabled
        # Adding answer here because it would be much useful than input question itself
        if self.zentry_memory:
            self.zentry_memory.add(data=answer, agent_id=self.config.id, user_id=session_id)
