...
### With Zentry to store chat history

Zentry is a cutting-edge long-term memory for LLMs to enable personalization for the GenAI stack. It enables LLMs to remember past interactions and provide more personalized responses. 

In order to use Zentry to enable memory for personalization in your apps:
- Install the [`Zentry`](https://docs.zentry.gg/) package using `pip install Zentryai`. 
...
## How Zentry works:
- Zentry saves context derived from each user question into its memory.
- When a user poses a new question, Zentry retrieves relevant previous memories.
- The `top_k` parameter in the memory configuration specifies the number of top memories to consider during retrieval.
- Zentry generates the final response by integrating the user's question, context from the data source, and the relevant memories.
