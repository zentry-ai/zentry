...
const configSlice = createSlice({
  ...
  reducers: {
    ...
    setConfigSuccess: (state, action: PayloadAction<{ openmemory?: OpenMemoryConfig; zentry: ZentryConfig }>) => {
      if (action.payload.openmemory) {
        state.openmemory = action.payload.openmemory;
      }
      state.zentry = action.payload.zentry;
    ...
    updateLLM: (state, action: PayloadAction<LLMProvider>) => {
      state.zentry.llm = action.payload;
    },
    updateEmbedder: (state, action: PayloadAction<EmbedderProvider>) => {
      state.zentry.embedder = action.payload;
    },
    updateZentryConfig: (state, action: PayloadAction<ZentryConfig>) => {
      state.zentry = action.payload;
    },
  },
});
