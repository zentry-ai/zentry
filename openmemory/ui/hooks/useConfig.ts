...
interface UseConfigApiReturn {
  ...
  saveConfig: (config: { openmemory?: OpenMemoryConfig; zentry: ZentryConfig }) => Promise<void>;
...
export const useConfig = (): UseConfigApiReturn => {
  ...
  const saveConfig = async (config: { openmemory?: OpenMemoryConfig; zentry: ZentryConfig }) => {
    setIsLoading(true);
    setError(null);
  ...
  const saveLLMConfig = async (llmConfig: LLMProvider) => {
    ...
    try {
      const response = await axios.put(`${URL}/api/v1/config/zentry/llm`, llmConfig);
  ...
  const saveEmbedderConfig = async (embedderConfig: EmbedderProvider) => {
    ...
    try {
      const response = await axios.put(`${URL}/api/v1/config/zentry/embedder`, embedderConfig);
