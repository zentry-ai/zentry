...
if [ -z "$OPENAI_API_KEY" ]; then
  echo "❌ OPENAI_API_KEY not set. Please run with: curl -sL https://raw.githubusercontent.com/zentryai/zentry/main/openmemory/run.sh | OPENAI_API_KEY=your_api_key bash"
...
# Check if the container "zentry_ui" already exists and remove it if necessary
if [ $(docker ps -aq -f name=zentry_ui) ]; then
  echo "⚠️ Found existing container 'zentry_ui'. Removing it..."
  docker rm -f zentry_ui
fi
...
services:
  ...
  openmemory-mcp:
    image: zentry/openmemory-mcp:latest
    environment:
      - OPENAI_API_KEY=${OPENAI_API_KEY}
      - USER=${USER}
      - QDRANT_HOST=zentry_store
      - QDRANT_PORT=6333
    depends_on:
      - zentry_store
    ports:
