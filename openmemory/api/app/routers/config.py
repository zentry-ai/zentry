...
def get_config_from_db(db: Session, key: str = "main"):
    ...
    if "zentry" not in config_value:
        config_value["zentry"] = default_config["zentry"]
    else:
        # Ensure LLM config exists with defaults
        if "llm" not in config_value["zentry"] or config_value["zentry"]["llm"] is None:
            config_value["zentry"]["llm"] = default_config["zentry"]["llm"]
        
        # Ensure embedder config exists with defaults
        if "embedder" not in config_value["zentry"] or config_value["zentry"]["embedder"] is None:
            config_value["zentry"]["embedder"] = default_config["zentry"]["embedder"]
...
async def update_configuration(config: ConfigSchema, db: Session = Depends(get_db)):
    ...
    # Update zentry settings
    updated_config["zentry"] = config.zentry.dict(exclude_none=True)
...
@router.get("/zentry/llm", response_model=LLMProvider)
async def get_llm_configuration(db: Session = Depends(get_db)):
    """Get only the LLM configuration."""
    config = get_config_from_db(db)
    llm_config = config.get("zentry", {}).get("llm", {})
    return llm_config

@router.put("/zentry/llm", response_model=LLMProvider)
async def update_llm_configuration(llm_config: LLMProvider, db: Session = Depends(get_db)):
    """Update only the LLM configuration."""
    current_config = get_config_from_db(db)
    
    # Ensure zentry key exists
    if "zentry" not in current_config:
        current_config["zentry"] = {}
    
    # Update the LLM configuration
    current_config["zentry"]["llm"] = llm_config.dict(exclude_none=True)
    
    # Save the configuration to database
    save_config_to_db(db, current_config)
    reset_memory_client()
    return current_config["zentry"]["llm"]

@router.get("/zentry/embedder", response_model=EmbedderProvider)
async def get_embedder_configuration(db: Session = Depends(get_db)):
    """Get only the Embedder configuration."""
    config = get_config_from_db(db)
    embedder_config = config.get("zentry", {}).get("embedder", {})
    return embedder_config

@router.put("/zentry/embedder", response_model=EmbedderProvider)
async def update_embedder_configuration(embedder_config: EmbedderProvider, db: Session = Depends(get_db)):
    ...
    current_config["zentry"]["embedder"] = embedder_config.dict(exclude_none=True)
    
    # Save the configuration to database
    save_config_to_db(db, current_config)
    reset_memory_client()
    return current_config["zentry"]["embedder"]
