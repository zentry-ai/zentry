...
from zentry import Memory, MemoryClient
from zentry.configs.prompts import MEMORY_ANSWER_PROMPT
from zentry.memory.telemetry import capture_client_event, capture_event
...
class Zentry:
    def __init__(
        ...
        if api_key:
            self.zentry_client = MemoryClient(api_key, host)
        else:
            self.zentry_client = Memory.from_config(config) if config else Memory()

        self.chat = Chat(self.zentry_client)


class Chat:
    def __init__(self, zentry_client):
        self.completions = Completions(zentry_client)


class Completions:
    def __init__(self, zentry_client):
        self.zentry_client = zentry_client
    ...
    def create(
        ...
        if isinstance(self.zentry_client, Memory):
            capture_event("zentry.chat.create", self.zentry_client)
        else:
            capture_client_event("zentry.chat.create", self.zentry_client)
        return response
    ...
    def _fetch_relevant_memories(self, messages, user_id, agent_id, run_id, filters, limit):
        ...
        return self.zentry_client.search(
            query="\n".join(message_input),
            user_id=user_id,
            agent_id=agent_id,
            run_id=run_id,
    ...
    def _format_query_with_memories(self, messages, relevant_memories):
        # Check if self.zentry_client is an instance of Memory or MemoryClient

        entities = []
        if isinstance(self.zentry_client, zentry.memory.main.Memory):
        ...
        elif isinstance(self.zentry_client, zentry.client.main.MemoryClient):
