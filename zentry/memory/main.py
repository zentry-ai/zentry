...
from zentry.configs.base import MemoryConfig, MemoryItem
from zentry.configs.enums import MemoryType
from zentry.configs.prompts import (
    PROCEDURAL_MEMORY_SYSTEM_PROMPT,
    get_update_memory_messages,
)
from zentry.memory.base import MemoryBase
from zentry.memory.setup import zentry_dir, setup_config
from zentry.memory.storage import SQLiteManager
from zentry.memory.telemetry import capture_event
from zentry.memory.utils import (
    get_fact_retrieval_messages,
    parse_messages,
    parse_vision_messages,
    remove_code_blocks,
)
from zentry.utils.factory import EmbedderFactory, LlmFactory, VectorStoreFactory
...
class Memory(MemoryBase):
    def __init__(self, config: MemoryConfig = MemoryConfig()):
        ...
        if self.config.graph_store.config:
            if self.config.graph_store.provider == "memgraph":
                from zentry.memory.memgraph_memory import MemoryGraph
            else:
                from zentry.memory.graph_memory import MemoryGraph

            self.graph = MemoryGraph(self.config)
            self.enable_graph = True
        else:
            self.graph = None
        self.config.vector_store.config.collection_name = "zentrymigrations"
        if self.config.vector_store.provider in ["faiss", "qdrant"]:
            provider_path = f"migrations_{self.config.vector_store.provider}"
            self.config.vector_store.config.path = os.path.join(zentry_dir, provider_path)
            os.makedirs(self.config.vector_store.config.path, exist_ok=True)
        self._telemetry_vector_store = VectorStoreFactory.create(
            self.config.vector_store.provider, self.config.vector_store.config
        )
        capture_event("zentry.init", self, {"sync_type": "sync"})
        ...
        *,
        ...
        if self.api_version == "v1.0":
            warnings.warn(
                ...
                "The current format will be removed in zentryai 1.1.0 and later versions.",
    ...
    def get(self, memory_id):
        ...
        """
        capture_event("zentry.get", self, {"memory_id": memory_id, "sync_type": "sync"})
    ...
    def get_all(
        ...
        *,
        ...
        capture_event(
            "zentry.get_all", self, {"limit": limit, "keys": list(effective_filters.keys()), "sync_type": "sync"}
        )
        ...
        if self.api_version == "v1.0":
            warnings.warn(
                ...
                "The current format (direct list for v1.0) will be removed in zentryai 1.1.0 and later versions.",
                category=DeprecationWarning,
                stacklevel=2,
            )
    ...
    def update(self, memory_id, data):
        ...
        Returns:
            dict: Updated memory.
        """
        capture_event("zentry.update", self, {"memory_id": memory_id, "sync_type": "sync"})
    ...
    def delete(self, memory_id):
        ...
        """
        capture_event("zentry.delete", self, {"memory_id": memory_id, "sync_type": "sync"})
    ...
    def delete_all(self, user_id: Optional[str] = None, agent_id: Optional[str] = None, run_id: Optional[str] = None):
        ...
        capture_event("zentry.delete_all", self, {"keys": list(filters.keys()), "sync_type": "sync"})
    ...
    def history(self, memory_id):
        ...
        """
        capture_event("zentry.history", self, {"memory_id": memory_id, "sync_type": "sync"})
    ...
    def _create_memory(self, data, existing_embeddings, metadata=None):
        ...
        capture_event("zentry._create_memory", self, {"memory_id": memory_id, "sync_type": "sync"})
        return memory_id
    ...
    def _create_procedural_memory(self, messages, metadata=None, prompt=None):
        ...
        capture_event("zentry._create_procedural_memory", self, {"memory_id": memory_id, "sync_type": "sync"})
    ...
    def _update_memory(self, memory_id, data, existing_embeddings, metadata=None):
        ...
        capture_event("zentry._update_memory", self, {"memory_id": memory_id, "sync_type": "sync"})
        return memory_id
    ...
    def _delete_memory(self, memory_id):
        ...
        capture_event("zentry._delete_memory", self, {"memory_id": memory_id, "sync_type": "sync"})
        return memory_id
    ...
    def reset(self):
        ...
        capture_event("zentry.reset", self, {"sync_type": "sync"})
...
class AsyncMemory(MemoryBase):
    def __init__(self, config: MemoryConfig = MemoryConfig()):
        ...
        capture_event("zentry.init", self, {"sync_type": "async"})
    ...
    async def _add_to_vector_store(
        ...
        capture_event(
            "zentry.add", self, {"version": self.api_version, "keys": list(filters.keys()), "sync_type": "async"}
        )
        return returned_memories
    ...
    async def get(self, memory_id):
        ...
        Returns:
            dict: Retrieved memory.
        """
        capture_event("zentry.get", self, {"memory_id": memory_id, "sync_type": "async"})
    ...
    async def get_all(
        ...
        *,
        ...
        capture_event(
            "zentry.get_all", self, {"limit": limit, "keys": list(effective_filters.keys()), "sync_type": "async"}
        )
    ...
    async def update(self, memory_id, data):
        ...
        Returns:
            dict: Updated memory.
        """
        capture_event("zentry.update", self, {"memory_id": memory_id, "sync_type": "async"})
    ...
    async def delete(self, memory_id):
        ...
        Args:
            memory_id (str): ID of the memory to delete.
        """
        capture_event("zentry.delete", self, {"memory_id": memory_id, "sync_type": "async"})
    ...
    async def delete_all(self, user_id=None, agent_id=None, run_id=None):
        ...
        capture_event("zentry.delete_all", self, {"keys": list(filters.keys()), "sync_type": "async"})
    ...
    async def history(self, memory_id):
        ...
        Returns:
            list: List of changes for the memory.
        """
        capture_event("zentry.history", self, {"memory_id": memory_id, "sync_type": "async"})
    ...
    async def _create_memory(self, data, existing_embeddings, metadata=None):
        ...
        capture_event("zentry._create_memory", self, {"memory_id": memory_id, "sync_type": "async"})
        return memory_id
    ...
    async def _create_procedural_memory(self, messages, metadata=None, llm=None, prompt=None):
        ...
        capture_event("zentry._create_procedural_memory", self, {"memory_id": memory_id, "sync_type": "async"})
    ...
    async def _update_memory(self, memory_id, data, existing_embeddings, metadata=None):
        ...
        capture_event("zentry._update_memory", self, {"memory_id": memory_id, "sync_type": "async"})
        return memory_id
    ...
    async def _delete_memory(self, memory_id):
        ...
        capture_event("zentry._delete_memory", self, {"memory_id": memory_id, "sync_type": "async"})
        return memory_id
    ...
    async def reset(self):
        ...
        capture_event("zentry.reset", self, {"sync_type": "async"})
