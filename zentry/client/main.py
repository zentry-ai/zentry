...
from zentry.memory.setup import get_user_id, setup_config
from zentry.memory.telemetry import capture_client_event
...
class MemoryClient:
    """Client for interacting with the Zentry API.

    This class provides methods to create, retrieve, search, and delete memories
    using the Zentry API.

    Attributes:
        api_key (str): The API key for authenticating with the Zentry API.
        host (str): The base URL for the Zentry API.
    ...
    def __init__(
        ...
        Args:
            api_key: The API key for authenticating with the Zentry API. If not provided,
                     it will attempt to use the ZENTRY_API_KEY environment variable.
            host: The base URL for the Zentry API. Defaults to "https://api.zentry.gg".
        ...
        """
        self.api_key = api_key or os.getenv("ZENTRY_API_KEY")
        self.host = host or "https://api.zentry.gg"
        self.org_id = org_id
        self.project_id = project_id
        self.user_id = get_user_id()

        if not self.api_key:
            raise ValueError("Zentry API Key not provided. Please provide an API Key.")
        ...
        if client is not None:
            ...
            self.client.headers.update({"Authorization": f"Token {self.api_key}", "Zentry-User-ID": self.user_id})
        else:
            self.client = httpx.Client(
                base_url=self.host,
                headers={"Authorization": f"Token {self.api_key}", "Zentry-User-ID": self.user_id},
                timeout=300,
            )
    ...
    def add(self, messages: List[Dict[str, str]], **kwargs) -> Dict[str, Any]:
        ...
        if kwargs.get("output_format") != "v1.1":
            ...
            warnings.warn(
                ...
                "Check out the docs for more information: https://docs.zentry.gg/platform/quickstart#4-1-create-memories",
                DeprecationWarning,
                stacklevel=2,
            )
        kwargs["version"] = "v2"
    ...
    def chat(self):
        """Start a chat with the Zentry AI. (Not implemented)
...
class AsyncMemoryClient:
    """Asynchronous client for interacting with the Zentry API.
    ...
    def __init__(
        ...
        if client is not None:
            ...
            self.async_client.headers.update({"Authorization": f"Token {self.api_key}", "Zentry-User-ID": self.user_id})
        else:
