...
import { ZentryConfigSettings } from './zentry-types';
...
const convertToZentryFormat = (messages: LanguageModelV1Prompt) => {
    try {
        ...
        console.error("Error in convertToZentryFormat:", error);
        return [];
    }
}

const searchInternalMemories = async (query: string, config?: ZentryConfigSettings, top_k: number = 5) => {
    try {
        ...
        const apiKey = loadApiKey({
            apiKey: (config&&config.zentryApiKey),
            environmentVariableName: "ZENTRY_API_KEY",
            description: "Zentry",
        });
        ...
        const response = await fetch('https://api.zentry.gg/v2/memories/search/', options);
        if (!response.ok) {
...
const addMemories = async (messages: LanguageModelV1Prompt, config?: ZentryConfigSettings) => {
    try {
        let finalMessages: Array<Message> = [];
        if (typeof messages === "string") {
            finalMessages = [{ role: "user", content: messages }];
        } else {
            finalMessages = convertToZentryFormat(messages);
        }
...
const updateMemories = async (messages: Array<Message>, config?: ZentryConfigSettings) => {
    try {
        const apiKey = loadApiKey({
            apiKey: (config&&config.zentryApiKey),
            environmentVariableName: "ZENTRY_API_KEY",
        ...
        const response = await fetch('https://api.zentry.gg/v1/memories/', options);
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
...
const retrieveMemories = async (prompt: LanguageModelV1Prompt | string, config?: ZentryConfigSettings) => {
    try {
...
const getMemories = async (prompt: LanguageModelV1Prompt | string, config?: ZentryConfigSettings) => {
    try {
        const message = typeof prompt === 'string' ? prompt : flattenPrompt(prompt);
        const memories = await searchInternalMemories(message, config);
...
const searchMemories = async (prompt: LanguageModelV1Prompt | string, config?: ZentryConfigSettings) => {
    try {
        const message = typeof prompt === 'string' ? prompt : flattenPrompt(prompt);
        const memories = await searchInternalMemories(message, config);
        return memories;
