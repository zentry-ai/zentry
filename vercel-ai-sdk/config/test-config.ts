...
export const testConfig = {
  ...
  userId: "zentry.gg-sdk-test-user-1134774",
  deleteId: "",
  providers: [
    {
      name: "openai",
  ...
  createTestClient: (provider: Provider) => {
    return createZentry({
      ...
      zentryApiKey: process.env.ZENTRY_API_KEY,
  ...
  fetchDeleteId: async function () {
    ...
    try {
      const response = await fetch('https://api.zentry.gg/v1/entities/', options);
  ...
  deleteUser: async function () {
    ...
    try {
      const response = await fetch(`https://api.zentry.gg/v1/entities/user/${this.deleteId}/`, options);
      if (!response.ok) {
